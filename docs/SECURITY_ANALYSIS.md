...
- **Mitigación Actual:** 
  1. Se bloquean los endpoints de control del túnel (`/api/host/start` y `/stop`) para requests que contengan la cabecera `X-Forwarded-For`. Esto evita que un invitado remoto apague o modifique el estado del túnel del anfitrión.
