...
DATA_DIR = os.path.join(DATA_DIR_BASE, 'casosdeuso')
...
def actualizar_proyecto(pid, datos):
    ...
    for p in proyectos:
        if p['id'] == pid:
            ...
            if 'titulo' in datos: p['titulo'] = datos['titulo']
            if 'descripcion' in datos: p['descripcion'] = datos['descripcion']
            if 'datos' in datos: p['datos'] = datos['datos']
