/* ════════════════════════════════════════════
   seleccion.js — Historias de Usuario (Proyectos)
   ════════════════════════════════════════════ */

const API = '/api/historias/proyectos';
...
document.getElementById('form-nuevo').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        mostrarToast(`Archivo "${proyecto.titulo}" creado.`, 'success');
        setTimeout(() => {
            window.location.href = `/historias?pid=${proyecto.id}`;
        }, 600);
    } catch {
...
function renderizarProyectos(proys) {
    ...
    list.querySelectorAll('.project-item').forEach(item => {
        item.addEventListener('click', (e) => {
            ...
            window.location.href = `/historias?pid=${item.dataset.pid}`;
        });
    });
