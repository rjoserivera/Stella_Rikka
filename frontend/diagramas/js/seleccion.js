...
const API = '/api/diagramas/proyectos';
...
if(document.getElementById('form-nuevo')) {
    document.getElementById('form-nuevo').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        setTimeout(() => { window.location.href = `/diagramas/editor?pid=${proyecto.id}`; }, 600);
    ...
    document.getElementById('input-import-json').addEventListener('change', async (e) => {
      ...
      reader.onload = async (event) => {
        try {
          ...
          setTimeout(() => { window.location.href = `/diagramas/editor?pid=${result.id}`; }, 600);
...
function abrirProyecto(pid) {
  window.location.href = `/diagramas/editor?pid=${pid}`;
}
