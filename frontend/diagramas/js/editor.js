...
const PID = P.get('pid');
if (!PID) location.href = '/diagramas/seleccion';
const API = `/api/diagramas/proyectos/${PID}`;
...
function port(n,pos){
  ...
  if(pos==='top')   return {x:cx,y:n.y};
  if(pos==='bottom')return {x:cx,y:n.y+(n.h||60)};
  if(pos==='left')  return {x:n.x,y:cy};
  if(pos==='right') return {x:n.x+(n.w||140),y:cy};
  return {x:cx,y:cy};
}
...
function startConn(id,pos,e){
  ...
  function up(ev){
    ...
    if(tNode&&tNode.dataset.id!==id) makeEdge(id,pos,tNode.dataset.id,'top');
...
function clickConnect(id){
  if(!cxFrom){cxFrom={id,pos:'bottom'};renderNodes();return;}
  if(cxFrom.id===id){cxFrom=null;renderNodes();return;}
  makeEdge(cxFrom.id,cxFrom.pos,id,'top');
  cxFrom=null; modo='select';
  document.getElementById('mode-connect').classList.remove('active');
  document.getElementById('mode-select').classList.add('active');
  renderNodes();
...
/* ── Drag desde Toolbox ── */
const DEFAULTS = {
  rectangulo:{txt:'Proceso',bg:'#1e293b',bc:'#6366f1',w:140,h:60},
  rombo:      {txt:'¿Decisión?',bg:'#f59e0b',bc:'#d97706',w:160,h:90},
  ovalo:      {txt:'Inicio/Fin',bg:'#10b981',bc:'#059669',w:130,h:60},
  paralelogramo:{txt:'E/S Datos',bg:'#3b82f6',bc:'#2563eb',w:150,h:60},
  cilindro:   {txt:'BD',bg:'#8b5cf6',bc:'#7c3aed',w:120,h:70},
  documento:  {txt:'Documento',bg:'#6b7280',bc:'#4b5563',w:140,h:65},
};
function setupDrop(){
...
function exportPNG(){
  ...
  const nodeSVG=E.nodos.map(n=>{
    const nx=n.x-minX,ny=n.y-minY,nw=n.w||140,nh=n.h||60;
    const s=n.estilo||{};
    const bg2=s.bg||'#1e293b',bc=s.bc||'#6366f1',bw=s.bw||2;
    const tc=s.tc||'#fff',fs=s.fs||14,fw=s.fw||'normal',fi=s.fi||'normal';
    return `<rect x="${nx}" y="${ny}" width="${nw}" height="${nh}" rx="8" fill="${bg2}" stroke="${bc}" stroke-width="${bw}"/>
...
async function init(){
  ...
  setupModes(); setupProps(); setupKeys(); setupName(); setupHelp();
  ...
  if(btnExportRikka) {
      btnExportRikka.addEventListener('click', async () => {
          try {
              ...
              const res = await fetch(`/api/diagramas/proyectos/${PID}/exportar`);
              ...
              a.href = url;
              a.download = `${E.config.titulo || 'Diagrama'}.rikka`;
              a.click();
              URL.revokeObjectURL(url);
