/* ════════════════════════════════════════════
   seleccion.js — Lógica pantalla de selección Kanban
   ════════════════════════════════════════════ */
...
document.getElementById('form-nuevo').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    // Guardar preferencia (reutilizando el endpoint general si existe)
    await fetch('/api/preferencias', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ultima_herramienta_usada: 'kanban',
        kanban: { ultimo_proyecto_abierto: proyecto.id }
      })
    }).catch(() => {});

    mostrarToast(`Proyecto "${proyecto.titulo}" creado exitosamente.`, 'success');
    setTimeout(() => {
      window.location.href = `/kanban/tablero?pid=${proyecto.id}`;
    }, 600);
...
function renderizarProyectos(proyectos) {
  ...
  list.querySelectorAll('.project-item').forEach(tarea => {
    tarea.addEventListener('click', (e) => {
      if (e.target.closest('.project-action-btn')) return;
      abrirProyecto(tarea.dataset.pid);
    });
...
function abrirProyecto(pid) {
  fetch('/api/preferencias', {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ kanban: { ultimo_proyecto_abierto: pid } })
  }).catch(() => {});
  window.location.href = `/kanban/tablero?pid=${pid}`;
}
...
function confirmarEliminar(pid, nombre) {
  ...
  document.getElementById('modal-body').innerHTML =
    `¿Estás seguro de que quieres eliminar <strong>"${escHtml(nombre)}"</strong>? Se eliminarán también todas sus funcionalidades. <strong>Esta acción no se puede deshacer.</strong>`;
...
if (btnImportarJson && inputImportJson) {
  ...
  inputImportJson.addEventListener('change', async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        window.location.href = `/kanban/tablero?pid=${result.id}`;
      }, 800);
