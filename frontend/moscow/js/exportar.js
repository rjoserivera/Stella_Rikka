/* ════════════════════════════════════════════
   exportar.js — Exportación PNG y CSV MoSCoW
   ════════════════════════════════════════════ */
...
async function exportarJSON() {
  ...
  try {
    ...
    const res = await fetch(`/api/moscow/proyectos/${pid}/exportar`);
...
async function exportarPNG() {
  ...
  try {
    const grid = document.getElementById('moscow-grid');
    ...
    canvas.toBlob(blob => {
      ...
      descargarBlob(blob, `moscow_${Date.now()}.png`);
...
function exportarCSV() {
  const items = window.MoscowApp.getTodos();
  if (!items.length) {
  ...
  descargarBlob(blob, `moscow_${Date.now()}.csv`);
