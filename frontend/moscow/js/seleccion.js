/* ════════════════════════════════════════════
   seleccion.js — Lógica pantalla de selección MoSCoW
   ════════════════════════════════════════════ */
...
document.getElementById('form-nuevo').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    await fetch('/api/preferencias', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ultima_herramienta_usada: 'moscow',
        moscow: { ultimo_proyecto_abierto: proyecto.id }
      })
    }).catch(() => {});

    mostrarToast(`Proyecto "${proyecto.titulo}" creado exitosamente.`, 'success');
    setTimeout(() => {
      window.location.href = `/moscow/matriz?pid=${proyecto.id}`;
    }, 600);
...
function abrirProyecto(pid) {
  fetch('/api/preferencias', {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ moscow: { ultimo_proyecto_abierto: pid } })
  }).catch(() => {});
  window.location.href = `/moscow/matriz?pid=${pid}`;
}
...
if (btnImportarJson && inputImportJson) {
  ...
  inputImportJson.addEventListener('change', async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        window.location.href = `/moscow/matriz?pid=${result.id}`;
      }, 800);
